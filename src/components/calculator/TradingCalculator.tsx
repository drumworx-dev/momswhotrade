...
function buildPDFHtml(state: CalculatorState, r: CalculatorResults): string {
  ...
  const priceDec = state.assetCategory === 'forex' ? 4 : 2;
  const fp       = (n: number) => sym + n.toLocaleString('en-US', { minimumFractionDigits: priceDec, maximumFractionDigits: priceDec });
  ...
  return (
    ...
    '<div class="row"><span class="lbl">Entry Price</span><span class="val">' + fp(r.entryPrice) + '</span></div>' +
    '<div class="row"><span class="lbl">Stop Loss</span><span class="val">' + fp(r.stopLossPrice) + '</span></div>' +
    '<div class="row"><span class="lbl">Take Profit</span><span class="val">' + fp(r.takeProfitPrice) + '</span></div>' +
...
export function TradingCalculator() {
  ...
  // Forex uses 4 decimal places (pip precision); everything else uses 2
  const priceDecimals = state.assetCategory === 'forex' ? 4 : 2;
  ...
  const handleRRClick = (rr: string) => {
    ...
    setState(prev => ({ ...prev, riskReward: rr, takeProfit: tp.toFixed(priceDecimals) }));
    setResults(null);
  };
    ...
    <div className="flex flex-col h-full bg-bg-primary">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-56">
        <div className="max-w-lg mx-auto flex flex-col gap-4">
          ...
          <button
            onClick={() => { setState(DEFAULT); setResults(null); setTpUserEdited(false); localStorage.removeItem(CALC_STORAGE_KEY); }}
            className="text-sm text-text-tertiary hover:text-text-secondary transition-colors py-1 text-center w-full"
          >
            ✕ Clear & Start Over
          </button>
                ...
                <div className="rounded-card overflow-hidden shadow-sm border border-gray-100" style={{ height: '288px' }}>
                  <div className="flex flex-col h-full">
                    ...
                    >
                      ...
                      <div className="relative z-10 flex items-start justify-between">
                        <div>
                          ...
                          <div className="text-sm font-mono font-semibold text-emerald-600">{fmtAmt(results.takeProfitPrice, sym, priceDecimals)}</div>
                        </div>
                    ...
                    <div className="flex-none flex items-center justify-between px-5 bg-gray-900" style={{ height: '44px' }}>
                      ...
                      <span className="font-mono font-bold text-white">{fmtAmt(results.entryPrice, sym, priceDecimals)}</span>
                    </div>
                    ...
                    >
                      ...
                      <div className="relative z-10 flex items-end justify-between">
                        <div>
                          ...
                          <div className="text-sm font-mono font-semibold text-red-500">{fmtAmt(results.stopLossPrice, sym, priceDecimals)}</div>
                        </div>
                        <div className="text-right">
